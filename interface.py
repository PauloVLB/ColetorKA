...
from leitor import selecionar_arquivo
...
def exibir_janela_principal():
    ...
    altura_janela = 100   # altura da janela principal
...
def exibir_opcoes(janela_principal):
    lista_opcoes = [('Planilha de alunos', 'Selecionar Arquivo'), 
                    ('Planilha da Khan Academy', 'Selecionar Arquivo'),
                    ('Selecionar Recomendações', 'Selecionar')]  # lista de opções e nome de seus botões
...
def criar_opcoes(lista_opcoes, janela_principal):
    for [idx, [titulo, botao]] in enumerate(lista_opcoes): # para cada opcao, enumerando-as
        ...
        botao_selecionar = Button(janela_principal,    
                                  text=botao,
                                  command=selecionar_arquivo)   # cria o botao, chama a funcao 'selecionar arquivo' ao ser clicado
        
        opcao.grid(sticky='w', column=0, row=2*idx)             # renderiza a label, alinha à esquerda (w = west)
        botao_selecionar.grid(sticky='e', column=1, row=2*idx)  # renderiza o botao, alinha à direita (e = east)
        
        ttk.Separator(janela_principal, orient="horizontal").grid(row=2*idx+1,column=0,columnspan=4, ipadx=200) # cria um separador 
